...
export class CacheProperties extends React.Component<{}, State> {
		...
		window.addEventListener("message", (event) => {
			...
			if (message.command === WebviewCommand.ParsedRedisResource) {
				const parsedRedisResource =
					message.value as ParsedRedisResource;
	...
	render(): JSX.Element | null {
		...
		const nonSslPort = redisResource.enableNonSslPort
			? redisResource.port
			: Strings.StrDisabled;
		const shardCountLabel =
			redisResource.shardCount > 0 ? (
				<GeneralPropertyLabel
					label={Strings.StrShardCount}
					value={redisResource.shardCount}
				/>
			) : null;
		...
		return (
			<div className="properties-container">
				...
				<div>
					...
					<CopyableTextField
						id="nonSslPort"
						label={Strings.StrNonSslPort}
						value={nonSslPort}
					/>
					<CopyableTextField
						id="sslPort"
						label={Strings.StrSslPort}
						value={redisResource.sslPort}
					/>
					<CopyableTextField
						id="resourceId"
				...
				<GeneralPropertyLabel
					label={Strings.StrSku}
					value={redisResource.sku}
				/>
				<GeneralPropertyLabel
					label={Strings.StrLocation}
					value={redisResource.location}
				/>
				<GeneralPropertyLabel
					label={Strings.StrRedisVersion}
					value={redisResource.redisVersion}
				/>
				<GeneralPropertyLabel
					label={Strings.StrProvisioningState}
					value={redisResource.provisioningState}
				/>
				{shardCountLabel}
			</div>
		);
