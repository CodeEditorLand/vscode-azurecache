...
export class CollectionView extends React.Component<{}, State> {
	...
	componentDidMount(): void {
		...
		window.addEventListener("message", (event) => {
			...
			if (message.command === WebviewCommand.CollectionData) {
				const { data, hasMore, clearCache } =
					message.value as CollectionWebviewData;
				...
				this.setState((prevState) => ({
					...
					currentValue: clearCache
						? undefined
						: prevState.currentValue,
					data: clearCache
						? selectableData
						: [...prevState.data, ...selectableData],
					hasMore,
					isLoading: false,
				}));
	...
	render(): JSX.Element | null {
		const { currentValue, data, hasMore, isLoading, type, key, size } =
			this.state;

		return (
			<div class="dataviewer-container">
