...
const classNames = mergeStyleSets({
	itemSelected: {
		...
		selectors: {
			"&:hover": {
				background:
					"var(--vscode-list-activeSelectionBackground) !important",
			},
		},
	},
	itemCell: [
...
export class CollectionList extends React.Component<Props, {}> {
	onRenderCell = (
		...
		if (this.props?.type === "zset" || this.props?.type === "hash") {
			header = (
				<div>
					<div class={itemIndexClass}>{item.id}</div>
				</div>
			);
		} else if (this.props?.type === "set" || this.props?.type === "list") {
			header = <div class={itemIndexClass}>{index}</div>;
		}
		...
		return (
			<div
				...
				<div class={classNames.itemContent}>
					{header}
					<div class={classNames.itemName}>{item.value}</div>
				</div>
			</div>
		);
	};
