...
describe("URI Utils", () => {
	...
	describe("createKeyContentUri", () => {
		...
		it("should sanitize URI path", () => {
			...
			assert.strictEqual(
				uri.path,
				"my-cache.redis.cache.windows.net/a_b_ c_d_e"
			);
		});
		...
		it("should generate valid URI for strings", () => {
			...
			assert.strictEqual(
				uri.path,
				"my-cache.redis.cache.windows.net/mykey"
			);
		...
		it("should generate valid URI for lists", () => {
			...
			assert.strictEqual(
				uri.path,
				"my-cache.redis.cache.windows.net/mykey[10]"
			);
			assert.strictEqual(uri.scheme, "azureCache");
		});
		...
		it("should generate valid URI for hashes", () => {
			...
			assert.strictEqual(
				uri.path,
				"my-cache.redis.cache.windows.net/mykey[hashfield]"
			);
			assert.strictEqual(uri.scheme, "azureCache");
		});
		...
		it("should generate valid URI for sorted sets", () => {
			...
			assert.strictEqual(
				uri.path,
				"my-cache.redis.cache.windows.net/mykey[-100]"
			);
			assert.strictEqual(uri.scheme, "azureCache");
		});
	});
	...
	describe("decodeUri", () => {
		it("should throw error if given bad query params", () => {
			const uriMissingQuery = vscode.Uri.parse(
				"azureCache:host.net/mykey",
				true
			);
			assert.throws(() => decodeUri(uriMissingQuery), Error);
			const uriMissingPayload = vscode.Uri.parse(
				"azureCache:host.net/mykey?badpayload=test",
				true
			);
