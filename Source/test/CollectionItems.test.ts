...
describe("CollectionItems", () => {
	...
	describe("RedisListItem", async () => {
		it("should load child elements in continuous manner", async () => {
			...
			sandbox
				.stub(RedisClient, "connectToRedisResource")
				.resolves(stubRedisClient);
			sandbox.stub(stubRedisClient, "llen").resolves(15);

			// Stub LRANGE calls
			const lrangeStub = sandbox.stub(stubRedisClient, "lrange");
			lrangeStub
				.withArgs("mylist", 0, 9, 0)
				.resolves(
					Array.from(Array(10).keys()).map((num) => num.toString())
				);
			lrangeStub
	...
	describe("RedisHashItem", async () => {
		it("should load child elements in continuous manner", async () => {
			...
			const secondScanRes: [string, string[]] = [
				"2",
			...
			hashStub
				.withArgs("myhash", "0", "MATCH", "*", 0)
				.resolves(firstScanRes);
			hashStub
				.withArgs("myhash", "1", "MATCH", "*", 0)
				.resolves(secondScanRes);
			hashStub
				.withArgs("myhash", "2", "MATCH", "*", 0)
				.resolves(thirdScanRes);
	...
	describe("RedisSetItem", async () => {
		it("should load child elements in continuous manner", async () => {
			...
			scanStub
				.withArgs("myset", "0", "MATCH", "*", 0)
				.resolves(firstScanRes);
			scanStub
				.withArgs("myset", "1", "MATCH", "*", 0)
				.resolves(secondScanRes);
			scanStub
				.withArgs("myset", "2", "MATCH", "*", 0)
				.resolves(thirdScanRes);
	...
	describe("RedisZSetItem", async () => {
		it("should load child elements in continuous manner", async () => {
			...
			const zcardStub = sandbox
				.stub(stubRedisClient, "zcard")
				.resolves(11);
