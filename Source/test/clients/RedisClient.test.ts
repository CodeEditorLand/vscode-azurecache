...
describe("RedisClient", () => {
	...
	describe("connectToRedisResource", () => {
		it("should attempt to connect to the Redis cache if given valid parameters", async () => {
			...
			await TestRedisClient.connectToRedisResource(
		...
		it("should not call connect() if error is encountered", () => {
			...
			const promise = TestRedisClient.connectToRedisResource(
				sampleParsedRedisResourceMissingKeys
			);
	...
	describe("Redis commands", () => {
		it("should return proper values when connected to cache", async () => {
			const client = await TestRedisClient.connectToRedisResource(
				sampleParsedRedisResource
			);
			...
			assert.deepStrictEqual(await client.scan("0", "MATCH", "*", 0), [
				"0",
				["someValue"],
			]);
			assert.deepStrictEqual(
				await client.hscan("someKey", "0", "MATCH", "*", 0),
				["0", ["someValue"]]
			);
			assert.deepStrictEqual(
				await client.sscan("someKey", "0", "MATCH", "*", 0),
				["0", ["someValue"]]
			);
			assert.deepStrictEqual(
				await client.zscan("someKey", "0", "MATCH", "*", 0),
				["0", ["someValue"]]
			);
			...
			assert.deepStrictEqual(await client.zrange("someKey", 0, 1), [
				"someValue",
				"0",
			]);
