...
describe("RedisResourceClient", () => {
	...
	const sampleRedisResource = {
		id: "/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/res-group/providers/Microsoft.Cache/Redis/my-cache",
	...
	describe("listNextResources", () => {
		it("should return next list of parsed Redis resources", async () => {
			...
			const redisResourceClient = new RedisResourceClient(rmClient);
			const resourceList =
				await redisResourceClient.listNextResources("link");
	...
	describe("getAccessKey", () => {
		it("should return the primary key if exists", async () => {
			...
			const redisResourceClient = new RedisResourceClient(rmClient);
			const accessKey = await redisResourceClient.getAccessKey(
				"res-group",
				"name"
			);
	...
	describe("getRedisResourceById", () => {
		it("should return the parsed Redis resource", async () => {
			...
			const parsedResource =
				await redisResourceClient.getRedisResourceByName(
					"res-group",
					"name"
				);
		...
		it("should throw error if unable to find resource", async () => {
			...
			assert.rejects(
				redisResourceClient.getRedisResourceById(resourceId),
				Error
			);
			assert(stubbedRedis.get.calledOnce);
		});
		...
		it("should throw error if given invalid resource ID", async () => {
			...
			assert.rejects(
				async () =>
					redisResourceClient.getRedisResourceById("invalid-id"),
				Error
			);
	...
	describe("getRedisResourceByName", () => {
		...
		it("should throw error if unable to find resource", async () => {
			...
			assert.rejects(
				redisResourceClient.getRedisResourceByName("res-group", "name"),
				Error
			);
			assert(stubbedRedis.get.calledOnce);
		});
